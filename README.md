

# 🖼️ Screenshot to Code  

**Screenshot to Code** is an AI-powered web tool that converts UI screenshots into clean, production-ready frontend code.  
Upload any design screenshot and instantly get structured **HTML, Tailwind CSS, or React components** — no manual recreation needed.

Built to save developers time, eliminate repetitive UI work, and accelerate prototyping.

<img width="1919" height="873" alt="image" src="https://github.com/user-attachments/assets/9917031b-1493-4dcb-b1ca-0065d848dbee" />
---

## 🎯 What is Screenshot to Code?

Screenshot to Code bridges the gap between **design and development**.

Instead of manually rebuilding UIs from screenshots or design mockups, developers can simply:

📸 Upload a screenshot  
🤖 Let AI analyze the layout with precision  
🧩 Get pixel-perfect frontend code instantly  

The generated code can be used as a starting point for websites, dashboards, and web apps — with exact colors, spacing, and typography.

---

## 🌟 Why This Exists

Every developer has faced this pain:

- Designers share screenshots instead of code  
- Rebuilding UI manually wastes hours  
- Small layout mistakes creep in  
- Repetitive HTML/CSS slows down projects  

**Screenshot to Code fixes this.**

✔ Faster UI development  
✔ Less manual effort  
✔ Rapid prototyping  
✔ Cleaner starting code  

---

## ⚡ How It Works

### 🔄 Processing Flow
```
┌────────────────────────────┐
│ Upload UI Screenshot       │
│          ↓                 │
│ AI Vision Analysis         │
│          ↓                 │
│ Layout & Style Detection   │
│          ↓                 │
│ Code Generation            │
│          ↓                 │
│ Live Preview / Copy Code   │
└────────────────────────────┘
```


### 👨‍💻 User Experience
1. Select your preferred framework (HTML, Tailwind, or React)
2. Upload a UI screenshot (PNG/JPG)
3. Click "Generate Code"
4. View generated code instantly
5. Copy to clipboard with one click

---

## 🛠️ Key Features

- 📸 **Screenshot upload support** — PNG, JPG, and more
- 🧠 **AI-powered precision** — Exact colors, spacing, and typography
- 🎨 **Multiple frameworks** — HTML+CSS, Tailwind CSS, React components
- ⚡ **Instant generation** — Get code in seconds
- 📋 **One-click copy** — Copy to clipboard instantly
- 🎯 **Pixel-perfect output** — Matches designs with maximum accuracy

---

## 🛠️ Tech Stack
```
┌─────────────────────────────────────────────┐
│ Frontend                                    │
│ React (Hooks-based architecture)           │
├─────────────────────────────────────────────┤
│ AI Engine                                   │
│ Google Gemini 2.5 Flash (Vision + Code)   │
├─────────────────────────────────────────────┤
│ Icons                                       │
│ Lucide React (Clean & lightweight)         │
├─────────────────────────────────────────────┤
│ Image Processing                            │
│ Base64 encoding + FileReader API           │
├─────────────────────────────────────────────┤
│ Code Generation                             │
│ HTML, CSS, Tailwind CSS, React JSX         │
└─────────────────────────────────────────────┘
```

### Why These Choices?

- **React:** Modern, component-based UI with hooks
- **Google Gemini AI:** Advanced vision capabilities + accurate code generation
- **Lucide React:** Beautiful, lightweight icon library
- **Multiple output formats:** Flexibility for different project needs

---

## 🚀 Getting Started

### Prerequisites
- Node.js (v14+)
- npm or yarn
- Google Gemini API key

### Installation

1. **Clone the repository**
```bash
   git clone https://github.com/Raksh0204/screenshot-to-code.git
   cd screenshot-to-code
```

2. **Install dependencies**
```bash
   npm install
```

3. **Set up environment variables**
   
   Create a `.env` file in the root directory:
```env
   REACT_APP_GEMINI_API_KEY=your_gemini_api_key_here
```

4. **Start the development server**
```bash
   npm start
```

5. **Open your browser**
   
   Navigate to `http://localhost:3000`

---

## 📝 Usage

1. **Select Framework**  
   Choose between HTML+CSS, Tailwind CSS, or React components

2. **Upload Screenshot**  
   Click the upload area and select your UI screenshot

3. **Generate Code**  
   Click "Generate Code" and wait for AI processing (usually 3-5 seconds)

4. **Copy & Use**  
   Copy the generated code and paste it into your project

---

## 🎨 Supported Output Formats

| Framework | Description |
|-----------|-------------|
| **HTML + CSS** | Vanilla HTML with inline or embedded CSS |
| **Tailwind CSS** | HTML with Tailwind utility classes |
| **React** | Functional React component with Tailwind CSS |

---

## 🚀 Future Enhancements

- [ ] Live preview within the app
- [ ] Support for Vue.js and Svelte
- [ ] Responsive design variants
- [ ] Component library integration
- [ ] Export to CodeSandbox/StackBlitz
- [ ] Figma plugin support
- [ ] Dark mode UI

---

## 📈 Use Cases

- **Rapid prototyping** — Turn mockups into code in seconds
- **Learning tool** — Understand how designs translate to code
- **Hackathons** — Speed up UI development during time-crunched events
- **Design handoff** — Bridge the gap between designers and developers
- **Legacy redesigns** — Recreate UIs from screenshots

---

## 🤝 Contributing

Contributions are welcome! Feel free to:

- Report bugs
- Suggest new features
- Submit pull requests
- Improve documentation

---

## 👨‍💻 Author

**Rakshitha U**  
🎓 B.E. – Electronics & Communication Engineering  
💻 Screenshot to Code Project

---
